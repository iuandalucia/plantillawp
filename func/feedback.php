...
<<?php echo $tag ?> <?php comment_class(empty( $args['has_children'] ) ? '' : 'parent') ?> id="comment-<?php comment_ID() ?>">
  ...
  <?php endif; ?>
    <div class="comment-author vcard">
      ...
      <div>
        ...
        <?php if ($comment->comment_approved == '0') : ?>
          <p><em class="comment-awaiting-moderation"><?php _e('Tu comentario está pendiente de moderación.') ?></em></p>
        <?php endif; ?>
      </div>
    </div>
    <div class="row">
    ...
    <div class="reply">
      <p class="text-right"><?php edit_comment_link(__('<span class="btn btn-default btn-info">Editar</span>'),' ','' );	?> <?php comment_reply_link(array_merge( $args, array('add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?></p>
    </div>
      <?php if ( 'div' != $args['style'] ) : ?>
    </div>
  <?php endif; ?>
